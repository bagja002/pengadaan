...
function BuatPengadaan() {
  const navigate = useNavigate();
  const token = "e" //localStorage.getItem("jwtToken");
  const [id_pegawaii, setIdPegawai] = useState(""); // Replace 'jwtToken' with your actual token key
  const id_jenis_pe9ngadaan = "2";
  const [isSelected, setIsSelected] = useState(false);
  const [isSelected1, setIsSelected1] = useState(false);
  const [isSelected2, setIsSelected2] = useState(false);
  const [formData, setFormData] = useState({
    paket_pengadaan: "",
    id_jenis_pengadaan: id_jenis_pe9ngadaan,
    id_rup: "",
    nilai_rab: "",
    nilai_tkdn: "",
    nilai_rup: "",
      ...
      {token ? (
        <>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div>
            <UploadIzinPrinsipStyle isSelected={isSelected} component="label" varian="contained">
            {isSelected ? 'File Dipilih' : 'Upload IzinPrinsip'}
            <UploadInput type="file" onChange={handleFileChange} />
            </UploadIzinPrinsipStyle>
            </div>
            <div>
              <UploadSpek isSelected1={isSelected1} component="label" varian="contained">
                {isSelected1 ? "File Dipilih" : "Upload Spek/KK"}
                <UploadInput type="file" onChange={handleFileChange1} />
              </UploadSpek>
            </div>
            <div>
              <UploadRAB isSelected2={isSelected2} component="label" varian="contained">
